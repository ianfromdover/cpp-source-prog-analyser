...
std::string& RelationshipConstraint::stripCharacters(std::string& str, const std::string& chars) {
...
Table RelationshipConstraint::getFullTable(QueryPkbVirtual &pkb){
...
bool RelationshipConstraint::isDefaultHeaders(std::vector<std::string> header) {
    return header == this->getDefaultHeaders();
}

Table RelationshipConstraint::getRelationshipTable(QueryPkbVirtual & pkb) {
    if (this->getNot()) {
