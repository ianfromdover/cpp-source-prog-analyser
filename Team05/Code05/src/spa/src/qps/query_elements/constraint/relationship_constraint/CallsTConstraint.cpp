...
//    }
    removeHeaders({HEADER_CALLSTLHS, HEADER_CALLSTRHS}, make_shared<ResultTable>(table));
...
std::size_t CallsTConstraint::hash() const {
    ...
    hashValue ^= stringHasher(CONSTRAINT_TYPE_CALLST) + HASH_OFFSET + (hashValue << 6) + (hashValue >> 2);
