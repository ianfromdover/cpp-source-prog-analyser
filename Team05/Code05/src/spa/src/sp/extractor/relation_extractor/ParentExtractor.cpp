...
void ParentExtractor::visitReadStmt(const Read& stmt, shared_ptr<Accumulator>& parentInfo) {
    updateParentInfo(stmt.getStmtNo(), parentInfo);
}
...
void ParentExtractor::updateParentInfo(const int stmtNo, std::shared_ptr<Accumulator>& parentInfo) {
    for (const auto& parentStmtNo : parentInfo->info) {
        pkb->addParentT(parentStmtNo, stmtNo);
    }
}