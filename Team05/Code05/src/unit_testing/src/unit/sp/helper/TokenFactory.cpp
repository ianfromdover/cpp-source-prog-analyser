...
TokenStream TokenFactory::createLogical(const TokenStream& left, TokenType op, const TokenStream& right) {
...
TokenStream TokenFactory::createBinary(const TokenStream& left, TokenType op, const TokenStream& right) {
    const auto stream = TokenFactory::createTokenStream({});
    stream->insert(stream->end(), left->begin(), left->end());
    stream->push_back(TokenFactory::createToken(op, TokenFactory::opToString(op)));
    stream->insert(stream->end(), right->begin(), right->end());
...
TokenStream TokenFactory::createTokenStream(const std::initializer_list<std::shared_ptr<Token>>& tokens) {
    return std::make_shared<std::vector<std::shared_ptr<Token>>>(tokens);
}

TokenStream TokenFactory::mergeStreams(const std::initializer_list<TokenStream>& streams) {
...
TokenStream TokenFactory::createProgram(const std::initializer_list<TokenStream>& procedures) {
...
TokenStream TokenFactory::createProcedure(const std::string& name, const std::initializer_list<TokenStream>& body) {
    const auto stream = TokenFactory::createTokenStream({
...
TokenStream TokenFactory::createGrouping(const TokenStream& expr) {
    const auto stream = TokenFactory::createTokenStream({
        TokenFactory::createToken(TokenType::LEFT_PAREN, "("),
    });
...
TokenStream TokenFactory::createAndExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createLogical(left, TokenType::AND, right);
}

TokenStream TokenFactory::createOrExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createLogical(left, TokenType::OR, right);
}

TokenStream TokenFactory::createNotExpr(const TokenStream& right) {
    const auto stream = TokenFactory::createTokenStream({
...
TokenStream TokenFactory::createAddExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::ADD, right);
}

TokenStream TokenFactory::createSubtractExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::SUBTRACT, right);
}

TokenStream TokenFactory::createMultiplyExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::MULTIPLY, right);
}

TokenStream TokenFactory::createDivideExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::DIVIDE, right);
}

TokenStream TokenFactory::createModExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::MOD, right);
}

TokenStream TokenFactory::createGreaterExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::GREATER, right);
}

TokenStream TokenFactory::createGreaterEqualExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::GREATER_EQUAL, right);
}

TokenStream TokenFactory::createLesserExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::LESSER, right);
}

TokenStream TokenFactory::createLesserEqualExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::LESSER_EQUAL, right);
}

TokenStream TokenFactory::createEqualsExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::EQUAL_EQUAL, right);
}

TokenStream TokenFactory::createNotEqualsExpr(const TokenStream& left, const TokenStream& right) {
    return TokenFactory::createBinary(left, TokenType::BANG_EQUAL, right);
}

TokenStream TokenFactory::createVariable(const std::string& name) {
    return TokenFactory::createTokenStream({
        TokenFactory::createToken(TokenType::NAME, name),
    });
}

TokenStream TokenFactory::createInt(int value) {
    return TokenFactory::createTokenStream({
        TokenFactory::createToken(TokenType::INTEGER, std::to_string(value)),
    });
}

TokenStream TokenFactory::createRead(const std::string& variable) {
    return TokenFactory::createTokenStream({
...
TokenStream TokenFactory::createPrint(const std::string& variable) {
    return TokenFactory::createTokenStream({
...
TokenStream TokenFactory::createCall(const std::string& procedure) {
    return TokenFactory::createTokenStream({
...
TokenStream TokenFactory::createWhile(const TokenStream& condition, const std::initializer_list<TokenStream>& body) {
    const auto stream = TokenFactory::createTokenStream({
...
TokenStream TokenFactory::createIf(const TokenStream& condition,
...
TokenStream TokenFactory::createAssign(const std::string& variable, const TokenStream& value) {
    const auto stream = TokenFactory::createTokenStream({
        TokenFactory::createToken(TokenType::NAME, variable),
